...
public class ArticleRepositoryImpl implements ArticleRepositoryCustom {
    ...
    @Override
    public Slice<Article> findNextPageByCursor(ArticleScrollRequest request) {
        QArticle article = QArticle.article;
        BooleanBuilder builder = buildArticleFilters(request, article);
    ...
    private BooleanBuilder buildArticleFilters(ArticleScrollRequest request, QArticle article) {
        ...
        if (request.getKeyword() != null && !request.getKeyword().isBlank()) {
            builder.and(
                    article.title.containsIgnoreCase(request.getKeyword())
        ...
        switch (request.getSortType()) {
            case POPULAR -> {
                if (request.getLastLikes() != null && request.getLastTitle() != null) {
                    builder.and(
                            article.likeCount.lt(request.getLastLikes())
                                    .or(article.likeCount.eq(request.getLastLikes())
                                            .and(article.title.lt(request.getLastTitle())))
                    );
                }
            }
            case LATEST -> {
                if (request.getLastUpdateTime() != null && request.getLastTitle() != null) {
                    builder.and(
                            article.createdAt.lt(request.getLastUpdateTime())
                                    .or(article.createdAt.eq(request.getLastUpdateTime())
                                            .and(article.title.lt(request.getLastTitle())))
                    );
                }
            }
